...
- **The in-app privacy wording:** the onboarding privacy screen says the app
  processes only messages that are pasted or shared, that the server holds the
  mobile number and the list of signed-in phones, and that records reach it
  only with Cloud sync, locked. Settings says messages are read on the phone.
